...
renumbered. When a decision is revised in full, a new ADR is
created with a `Supersedes:` reference; the older ADR's status
changes to `Superseded by ADR-NNN`. When a decision is revised in
part, the newer ADR states which clauses it displaces, and the older
ADR keeps status `Accepted` with a qualifier: `Accepted; superseded
in part by ADR-NNN` when named clauses are displaced, or `Accepted;
amended by ADR-NNN` when wording, examples, or mechanism details are
revised without the decision being displaced.
...
sharpens or supersedes it, never by editing the original decision.
The revision is made visible on the older ADR without touching its
decision text: a `> **Superseded in part by ADR-NNN:**` or
`> **Amended by ADR-NNN:**` blockquote near the top for a partial
revision, or a status change for a full supersession. Dated,
append-only `## Amendment` sections inside an ADR are part of that
ADR and do not change its status; only revision by another ADR does.
Each accepted ADR drives the next implementation phase, either as a
single instruction or as its own roadmap.
...
section. A CI guard (`make lint-doc`) fails when an ADR file is not
...
| 005 | [Hardened container profile, not lane-configurable](ADR-005-hardened-container-profile-non-configurable.md) | Accepted; amended by ADR-022, ADR-033 | No root; Code is liability; Peers are declared; Enforcement is structural |
...
| 008 | [Cryptographic primitives -- ECDSA P-256, scrypt + secretbox, crypto/rand](ADR-008-cryptographic-primitives.md) | Accepted; superseded in part by ADR-032; amended by ADR-043 | Digest-pinned references; Code is liability; Identity is asymmetric |
...
| 033 | [SSH Peer Egress and Unified Capsule Mediation](ADR-033-ssh-peer-egress-and-unified-mediation.md) | Accepted; superseded in part by ADR-038 | Peers are declared; Identity is asymmetric; No root; Runtime is attested; Code is liability; Enforcement is structural |
...
| 035 | [Build payload stays in the engine; outputs are workdir-volume projections](ADR-035-build-payload-in-engine.md) | Accepted; amended by ADR-036, ADR-046 | Containers are the only storage; Enforcement is structural; Reproducibility; Digest-pinned references; CUE first; Code is liability |
| 036 | [Engine-native step input delivery](ADR-036-engine-native-input-delivery.md) | Accepted; amended by ADR-046 | Containers are the only storage; Digest-pinned references; Reproducibility; Code is liability; Enforcement is structural |
| 037 | [Two trust layers toward the engine](ADR-037-two-engine-trust-layers.md) | Accepted; amended by ADR-041 | Runtime is attested; Identity is asymmetric; Code is liability; Reproducibility; Enforcement is structural; Observation over declaration |
...
| 039 | [The deploy step as attestation root](ADR-039-deploy-step-as-attestation-root.md) | Accepted; amended by ADR-042, ADR-051 | Code is liability; CUE first; Reproducibility; Observation over declaration |
| 040 | [Control-plane SBOM generation and keyless attestation](ADR-040-control-plane-sbom-and-keyless-attestation.md) | Accepted; amended by ADR-051 | Runtime is attested; Identity is asymmetric; Peers are declared; Digest-pinned references; Reproducibility; Code is liability; No exec |
...
| 048 | [Contract type semantics: naming by role, representation-neutral concepts, and the primitive/concept layering](ADR-048-contract-type-semantics.md) | Accepted; superseded in part by ADR-044 | CUE first; Meaning is single-sourced; Enforcement is structural; Code is liability |
...
| 051 | [Deploy is the sealing point -- a signed, SBOM'd subject over the deployed payload](ADR-051-deploy-as-sealing-point.md) | Accepted; amended by ADR-054 | Runtime is attested; Digest-pinned references; Reproducibility; Identity is asymmetric; Enforcement is structural; Code is liability |
...
ADR-041, ADR-042, ADR-047, ADR-048, ADR-049.
...
ADR-026, ADR-035, ADR-036, ADR-046.
...
ADR-044, ADR-047, ADR-048, ADR-049.
...
- **Status**: `Proposed`, `Accepted`, `Deprecated`, or `Superseded by
  ADR-NNN`. `Accepted` may carry revision qualifiers mirroring the
  notes on the ADR itself: `Accepted; superseded in part by ADR-NNN`
  (a later ADR displaces named clauses) and `Accepted; amended by
  ADR-NNN` (a later ADR revises wording, examples, or mechanism
  details without displacing the decision). A qualifier names every
  revising ADR; the substance of each revision lives only in the
  note on the ADR, and the status column above mirrors those notes.
