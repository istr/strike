...
**Rule.** Structs are declared in fieldalignment-clean order (largest
type first, then by descending size). For JSON unmarshaling, the JSON tag
does the mapping; field order is irrelevant to behaviour.
...
**Rationale.** `json.Unmarshal` is tag-driven. Declaring the struct in
the JSON's field order is a comment-as-code mechanism that incurs an
alignment cost for zero behavioural benefit. The justification "matches
the API response" is factually wrong about `Unmarshal`.

**Discovery.** `golangci-lint run` with `govet.enable-all`.
