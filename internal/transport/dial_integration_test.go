...
// TestDialVerified_HarnessDoT_INTEGRATION connects to the local
...
// address is an IP literal, so no SNI is sent and the certificate
// is accepted on its IP entry.
//
// The harness is a prerequisite: the test runs by default and
// fails fast when it is down; set STRIKE_INTEGRATION=0 to skip.
func TestDialVerified_HarnessDoT_INTEGRATION(t *testing.T) {
	...
	defer cancel()
	conn, err := transport.DialVerified(ctx, endpoint.MustParseAuthority("127.0.0.1:8853"), trust)
	if err != nil {
