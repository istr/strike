...
func TestParse_PeersReject(t *testing.T) {
	tests := []struct {
		...
		{"invalid_https_unknown_mode.yaml", "mode"},
		{"invalid_https_not_self_signed.yaml", "self-signed"},
		{"invalid_https_ca_false.yaml", "CA:FALSE"},
		{"invalid_https_cert_not_der.yaml", "DER certificate"},
		{"invalid_https_cert_not_base64.yaml", "trust.cert"},
		{"invalid_https_uppercase_host.yaml", "host"},
		{"invalid_https_underscore_host.yaml", "host"},
		{"invalid_missing_resolver.yaml", "resolver"},
		{"invalid_missing_oidc.yaml", "oidc"},
		{"invalid_peer_unknown_type.yaml", "type"},
		{"invalid_resolver_ip_is_hostname.yaml", "ip"},
		{"invalid_resolver_ip_not_parseable.yaml", "ip"},
		{"invalid_resolver_ip_not_canonical.yaml", "not canonical"},
		{"invalid_resolver_adn_is_ip.yaml", "adn"},
		{"invalid_resolver_leaf_trust.yaml", "rootca"},
